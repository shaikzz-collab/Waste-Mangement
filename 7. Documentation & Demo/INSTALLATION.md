...
Create a `.env` file in the `7. Documentation & Demo/src/backend/` directory and configure the variables:
```env
PORT=5000
...
Create a `.env` file in the `7. Documentation & Demo/src/frontend/` directory:
```env
...
From the **`7. Documentation & Demo/src`** directory, run:
```bash
...
From the **`7. Documentation & Demo/src/frontend`** directory, run:
```bash
npm run dev
```
